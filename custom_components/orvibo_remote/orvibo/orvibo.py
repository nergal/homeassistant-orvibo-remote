...
class Packet:
    ...
    def __repr__(self):
        return "Packet {} {}: {}".format(
            "to" if self.type == self.Request else "from",
            self.ip,
            _debug_data(self.data),
        )
...
class Orvibo(object):
    ...
    def __init__(self, ip, mac=None, type="Unknown"):
        self.ip = ip
        self.type = type
        self.__last_subscr_time = (
            time.time() - 1
        )  # Orvibo doesn't like subscriptions frequently that 1 in 0.1sec
    ...
    def __repr__(self):
        ...
        return "Orvibo[type={}, ip={}, mac={}]".format(
            self.type,
            "Unknown" if self.ip == BROADCAST else self.ip,
            mac.decode("utf-8") if py3 else mac,
        )
    ...
    def discover(ip=None):
        ...
        with _orvibo_socket() as s:
            ...
            for indx in range(
                512
            ):  # supposer there are less then 512 devices in the network
                ...
                logger.debug(
                    "Discovered values: type={}, mac={}".format(orvibo_type, orvibo_mac)
                )
        ...
        if ip not in devices.keys():
            raise OrviboException(
                "Device ip={} not found in {}.".format(ip, devices.keys())
            )
    ...
    def __subscribe(self, s):
        ...
        subscr_packet.compile(
            SUBSCRIBE, self.mac, SPACES_6, _reverse_bytes(self.mac), SPACES_6
        )
    ...
    def __control_s20(self, switchOn):
        ...
        with _orvibo_socket(self.__socket) as s:
            ...
            if self.type != Orvibo.TYPE_SOCKET:
                self.__logger.warn(
                    "Attempt to control device with type {} as socket.".format(
                        self.type
                    )
                )
                return False
            ...
            if curr_state == state:
                self.__logger.warn(
                    "No need to switch {0} device which is already switched {0}".format(
                        "on" if switchOn else "off"
                    )
                )
                return False

            self.__logger.debug(
                "Socket is switching {}".format("on" if switchOn else "off")
            )
            ...
            if on_off_packet.recv(s, CONTROL_RESP) is None:
                self.__logger.warn(
                    "Socket switching {} failed.".format("on" if switchOn else "off")
                )
                return False

            self.__logger.info(
                "Socket is switched {} successfuly.".format("on" if switchOn else "off")
            )
            return True
    ...
    def learn(self, fname=None, timeout=15):
        ...
        with _orvibo_socket(self.__socket) as s:
            if self.__subscribe(s) is None:
                self.__logger.warn(
                    "Subscription failed while entering to Learning IR/RF433 mode"
                )
                return

            if self.type != Orvibo.TYPE_IRDA:
                self.__logger.warn(
                    "Attempt to enter to Learning IR/RF433 mode for device with type {}".format(
                        self.type
                    )
                )
                return
            ...
            learn_packet = Packet(self.ip).compile(
                LEARN_IR, self.mac, SPACES_6, b"\x01\x00", ZEROS_4
            )
            learn_packet.send(s)
            ...
            while True:
                ...
                if packet_with_signal is None:
                    self.__logger.info(
                        "The rest time: {} sec".format(int(timeout - elapsed_time))
                    )
                    continue
                ...
                if packet_with_signal.length == EMPTY_LEARN_IR:
                    self.__logger.debug(
                        "Skipped:\nEmpty packet = {}".format(
                            _debug_data(packet_with_signal.data)
                        )
                    )
                    continue
            ...
            if fname is not None:
                with open(fname, "wb") as f:
                    f.write(signal)
                self.__logger.info(
                    'IR/RF433 signal got successfuly and saved to "{}" file'.format(
                        fname
                    )
                )
            else:
    ...
    def _learn_emit_rf433(self, on, key):
        ...
        with _orvibo_socket(self.__socket) as s:
            ...
            signal_packet = Packet(self.ip).compile(
                BLAST_RF433,
                self.mac,
                SPACES_6,
                key[:4],
    ...
    def emit_ir(self, signal):
        ...
        with _orvibo_socket(self.__socket) as s:
            ...
            if self.type != Orvibo.TYPE_IRDA:
                self.__logger.warn(
                    "Attempt to emit IR signal for device with type {}".format(
                        self.type
                    )
                )
                return False
            ...
            signal_packet = Packet(self.ip).compile(
                BLAST_IR, self.mac, SPACES_6, b"\x65\x00\x00\x00", _packet_id(), signal
            )
            signal_packet.send(s)
            signal_packet.recv_all(s)
...
def usage():
    print(
        "orvibo.py [-v] [-L <log level>] [-i <ip>] [-m <mac> -x <irda|socket>] [-s <on/off>] [-e <file.ir>] [-t <file.ir>] [-r]"
    )
    ...
    print(
        "-t <fname> - turns Orvibo AllOne into learning mode for 15 seconds or until catching IR signal"
    )
...
if __name__ == "__main__":
    ...
    class Opts:
        ...
        def discover_all(self):
            return (
                self.ip is None
                and self.mac is None
                and self.switch is None
                and self.emitFile is None
                and self.teachFile is None
            )
